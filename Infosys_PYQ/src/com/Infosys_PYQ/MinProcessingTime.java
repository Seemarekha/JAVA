...
public class MinProcessingTime {
	...
	public static void main(String[] args) {
		...
		System.out.println("Minimum processing time : "+minProcessingTime(processorTime, tasks));  // 16
	}

	private static int minProcessingTime(List<Integer> processorTime, List<Integer> tasks) {
		...
		for (int i = 0; i < processorTime.size(); i++) {
			int currMaxTask = tasks.get(j);
			max = Math.max(max, currMaxTask + processorTime.get(i));
